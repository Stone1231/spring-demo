...
public class RedissonCacheAspect extends AbstractCommonService {
	...
	private static transient final Logger LOGGER = LoggerFactory.getLogger(RedissonCacheAspect.class);
	...
	/**
	 * redisMode = 1:sentinel , 2:cluster
	 * defualt 1
	 */
	 ...
	 */
	private transient Map<String, RedissonClient> redissonClients = new HashMap<String, RedissonClient>();
	...
	protected void init() {
		...
		//
		for (Map.Entry<String, RedissonManager> entry : cacheManagers.entrySet()) {
	...
	public String getClusterCacheManagerName() {
		return clusterCacheManagerName;
	}

	public void setClusterCacheManagerName(String clusterCacheManagerName) {
		this.clusterCacheManagerName = clusterCacheManagerName;
	}
	 ...
	 * @return
	 */
	public RedissonManager getCacheManager(String cacheManager, String dbId) {
		...
		// LOGGER.debug("CacheManager : " + managerName + " : " + result);
		return result;
	}
	...
	public Object cacheable(ProceedingJoinPoint pjp) throws Throwable {
		...
		LOGGER.info("Redisson Cacheable Join point : " + getProceedName(pjp, method));
