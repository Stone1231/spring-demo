...
public final class NumberUtil {
	...
	//
	public static final Float FLOAT_ZERO = Float.valueOf(0f);

	public static final Float FLOAT_ONE = Float.valueOf(1f);
	...
	protected static final int[] RETRY_BACKOFF = { 1, 1, 1, 2, 2, 4, 4, 8, 16, 32 };
	...
	protected static class Static {
		public Static() {
			try {
				...
				LOGGER.error("Exception encountered during Static()", ex);
			}
		}
	}
	...
	public static BigDecimal createBigDecimal(Object value, double defaultValue, String pattern, Locale locale) {
		...
		try {
			...
			LOGGER.error("Exception encountered during createBigDecimal()", ex);
		}
	...
	public static Byte[] toBytes(byte[] values, int defaultValue) {
		...
		if (values != null) {
			...
			for (int i = 0; i < results.length; i++) {
				byte element = values[i];
				try {
					results[i] = Byte.valueOf(element);
				} catch (Exception ex) {
	...
	public static Short[] toShorts(short[] values, int defaultValue) {
		...
		if (values != null) {
			...
			for (int i = 0; i < results.length; i++) {
				short element = values[i];
				try {
					results[i] = Short.valueOf(element);
				} catch (Exception ex) {
	...
	public static Integer[] toInts(int[] values, int defaultValue) {
		...
		if (values != null) {
			...
			for (int i = 0; i < results.length; i++) {
				...
				try {
					results[i] = Integer.valueOf(element);
				} catch (Exception ex) {
	...
	public static Long[] toLongs(long[] values, long defaultValue) {
		...
		if (values != null) {
			...
			for (int i = 0; i < results.length; i++) {
				long element = values[i];
				try {
					results[i] = Long.valueOf(element);
				} catch (Exception ex) {
					results[i] = defaultValue;
					//
				}
	...
	public static Float[] toFloats(float[] values, float defaultValue) {
		...
		if (values != null) {
			...
			for (int i = 0; i < results.length; i++) {
				float element = values[i];
				try {
					results[i] = Float.valueOf(element);
				} catch (Exception ex) {
					results[i] = defaultValue;
					//
				}
	...
	public static Double[] toDoubles(double[] values, double defaultValue) {
		...
		if (values != null) {
			...
			for (int i = 0; i < results.length; i++) {
				double element = values[i];
				try {
					results[i] = Double.valueOf(element);
				} catch (Exception ex) {
					results[i] = defaultValue;
					//
				}
	...
	public static double divide(Object value1, Object value2, int scale, double defaultValue1, double defaultValue2,
		...
		BigDecimal big1 = createBigDecimal(value1, defaultValue1, pattern, locale);
		BigDecimal big2 = createBigDecimal(value2, defaultValue2, pattern, locale);
		return ((BigDecimal.ZERO.compareTo(big2) == 0) ? 0
				: big1.divide(big2, scale, BigDecimal.ROUND_HALF_UP).doubleValue());
	}
	...
	public static long retryPause(int tries, long retryPauseMills) {
		...
		if (triesCount >= RETRY_BACKOFF.length) {
			triesCount = RETRY_BACKOFF.length - 1;
		}
		return (retryPauseMills * RETRY_BACKOFF[triesCount]);
	}
}
